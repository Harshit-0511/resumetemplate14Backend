...
const fetchResume = useCallback(async () => {
  ...
  const templateId =  "resumeTemplate14" //"temp14"; or dynamic
...
const handleSaveResume = useCallback(
  async (showToast = true) => {
    try {
      const payload = {
        templateId: "resumeTemplate14", //"temp14", update this dynamically if needed
        ...resumeData,
      };
...
//   inhance resume section like summary,experience,project,course etc .
  ...
  async (field) => {
    ...
    try {
      ...
      const payload = {
        ...
        templateId: "resumeTemplate14",//"temp14",
        field,
        data:
          field === "experience" ? resumeData.experience : resumeData[field],
      };

      console.log("Payload for enhancement:", JSON.stringify(payload, null, 2));
...
useEffect(() => {
  ...
  setIsAiDropdownOpen(false); // AI Assistant dropdown band karo

  // Ab yahan decide karo ki AI enhance karna hai ya sirf tips dikhane hain
  if (section === 'summary') {
    enhanceSingleField('summary'); // Summary field ko AI se enhance karo
  } else if (section === 'experience') {
    enhanceSingleField('experience'); // Experience field ko AI se enhance karo
  } else if (section === 'projects') {
    enhanceSingleField('projects'); // Projects field ko AI se enhance karo
  } else {
    // Agar koi aur section hai (jaise 'general' tips) ya unknown section,
    // toh sirf AI Assistant Modal (tips wala) open karo
